...
of lightning-training-studio and a [feature branch](https://github.com/JustinGoheen/lightning-pod/tree/feature/add-training-studio)
of lightning-pod.
...
See lightning-pod's [optuna-flow](https://github.com/JustinGoheen/lightning-pod/blob/master/lightning_pod/flows/optuna_flow.py)
for an example of HPO with Lightning and Optuna that logs grouped trials to Weights
& Biases.
...
In the example [lightning_pod.core.module](https://github.com/JustinGoheen/lightning-pod/blob/master/lightning_pod/core/module.py), the hyperparameters are `lr`, `dropout`, and `optimizer` where:

- `lr` is the learning rate applied to the optimizer
- `dropout` is the probability of an element to be zeroed
...
See [Conv-TasNet](https://github.com/naplab/Conv-TasNet/blob/master/conv_tasnet.py) as an example of using keyword arguments as a means of setting outer and inner layer hyperparameters.
